...
const redisPort = parseInt(parsed.port || '6379', 10)
...
async function startWorker() {
  await new Promise<void>((resolve) => {
    ...
    connection.once('error', () => {
      console.warn('Redis connection error during startup, BullMQ will retry when available')
      resolve()
    })
  })

  console.log(`Starting BullMQ Worker with Redis ${redisHost}:${redisPort}`)
  ...
  }, { connection: { host: redisHost, port: redisPort } as any })

  workerInstance.on('completed', (job) => console.log('Job completed', job.id))
  workerInstance.on('failed', (job, err) => console.error('Job failed', job?.id, err))
...
async function shutdown(signal?: string) {
  ...
  try {
    ...
    if (workerInstance) {
      try { await workerInstance.close() } catch (e) {}
    }
    process.exit(0)
  } catch (e) {
